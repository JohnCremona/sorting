...
The Dedekind-Kummer theorem allows us to associate to each factor $h_i(X)$ the prime ideal $\p_i=(p,h_i(\alpha))$ above $p$,
with residue degree $f(\p_i/p)=\deg h_i$ and norm $N(\p_i)=p^{\deg h_i}$.
We then have $p\OO_K=\prod_i \p_i$ and our ordering of the $h_i$ induces an ordering of the primes~$\p_i$ above $p$.
...
= e(\p/p)f(\p/p)$ \cite{something-or-prove-it}.  Write
\[
...
to~$h$, and if $k$ is such that the set $\{v_\p(h_i(\alpha))\}$ has a unique maxima, this maxima occurs for the polynomial $h_i(X)$ corresponding to $\p$.
Thus we can compute the bijection by making $k$ sufficiently large (and any $k$ that yields a unique maxima works).
...
\subsection{Examples}
Let $K=\Q(\alpha)$ be non-Galois cubic field \href{www.lmfdb.org/NumberField/3.1.503.1}{3.1.503.1} of discriminant $-503$ with canonical defining polynomial $g(x)=X^3-X^2+2X+8$.
...
primes above $2$, each with residue degre $1$.
...
We illustrate the order on $p$-adic polynomials on another example. Let~$g(x) =
x^{10} - 3x^9 - 35x^8 + 120x^7 + 242x^6 - 1080x^5 + 44x^4 + 2343x^3 -
1631x^2 + 111x + 79$, which is irreducible over~$\Q$. Over~$\Q_3$, the polynomial $g(X)$ splits as
