...
write
\[
    g(X) \equiv \prod_{i=1}^{g}h_i(X) \pmod{p},
\]
...
coefficients of each $h_i(X)$ are reduced modulo~$p$ to lie in
$\{0,1,\dots,p-1\}$, and we order these factors first by degree and
then lexicographically according to their coefficient vectors in $\{0,\ldots,p-1\}^f$.
...
To compute the associated sorting key attached to a prime~$\p$
above~$p$: factor $g(X)$ modulo~$p$ as above, let $h(X)$ be the unique
...
the irreducible factors $h(X)$ of $g(X)$ in $\Q_p[X]$, such that $\deg h
= e(\p/p)f(\p/p)$.  Write
\[
...
of the same degree (and in fact we are interested only in those polynomials $h_i(X)$
...
Note that the $p$-adic polynomial factorisation algorithm Round 4 actually
...
$g(X) = X^3+X^2-2X+8$ be the minimal polynomial of~$\alpha$, with discriminant
$\disc g = -2^2\cdot503$.
